(function(module) {
  ...
  Article.createTable = function() {
    ...
    webDB.execute(
      'CREATE TABLE IF NOT EXISTS articles (id INTEGER PRIMARY KEY, title VARCHAR, category VARCHAR, author VARCHAR, authorUrl VARCHAR, publishedOn VARCHAR, body VARCHAR);', // TODONE: What SQL command do we run here inside these quotes?
      function() {
  ...
  Article.loadAll = function(rows) {
    Article.allArticles = rows.map(function(ele) {
      ...
      console.log('Article.loadAll has fired');
    });
  };
  ...
  Article.prototype.insertRecord = function() {
    webDB.execute(
      [{
        ...
        'sql': 'INSERT INTO articles (title, category, author, authorUrl, publishedOn, body) VALUES(?, ?, ?, ?, ?, ?);', // <----- TODONE: complete our SQL query here, inside the quotes.
  ...
  Article.fetchAll = function(nextFunction) {
    webDB.execute(
      'SELECT * FROM articles;', // <-----TODONE: fill these quotes to query our table.
      function(rows) {
        ...
        if (rows.length) {
          ...
          $.getJSON('/data/hackerIpsum.json', function(responseData) {
            ...
            webDB.execute(
              'SELECT * FROM articles;', // <-----TODONE: query our table for articles once more
              function(rows) {
                // TODONE:
      ...
      [
        {
          ...
          'sql': 'DELETE FROM articles WHERE id = ?', // <---TODONE: Delete an article instance from the database based on its id:
          'data': [this.id]
        }
      ]
    );
  ...
  Article.clearTable = function() {
    webDB.execute(
      'DELETE FROM articles;' // <----TODONE: delete all records from the articles table.
    );
  };
